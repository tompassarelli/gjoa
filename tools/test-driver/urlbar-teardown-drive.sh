...
# revamp mode expanded, drive urlbar-teardown-probe.py, tear down.
set -u
SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
ROOT=$(cd -- "$SCRIPT_DIR/../.." && pwd)
BIN="$ROOT/engine/obj-x86_64-pc-linux-gnu/dist/bin/gjoa"
PROBE="$ROOT/tools/test-driver/urlbar-teardown-probe.py"
PORT="${PORT:-3091}"
OUTDIR="${OUTDIR:-/tmp/urlbar-teardown}"
...
python3 "$PROBE" --port "$PORT" --settle-ms 5000
rc=$?
kill "$gpid" 2>/dev/null; sleep 1; kill -9 "$gpid" 2>/dev/null
disarm_render_ws
rm -rf "$prof"
echo "urlbar-teardown probe done"
exit $rc
